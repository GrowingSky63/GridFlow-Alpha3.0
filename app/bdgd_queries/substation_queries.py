...
class SubstationQueryMixin:
    ...
    def get_all_substations(
        ...
        return self._select_many_with_geometry(self.substation_table, None, limit, offset, mapped)  # type: ignore

    def get_substation_by_id(self, id: int, mapped: bool = True):
        where = self.substation_table.c.id == id
        return self._select_one_with_geometry(self.substation_table, where, mapped)  # type: ignore

    def get_substation_by_cod_id(self, cod_id: str, mapped: bool = True):
        where = self.substation_table.c.cod_id == cod_id
        return self._select_one_with_geometry(self.substation_table, where, mapped)  # type: ignore

    def get_substation_by_name(self, name: str, mapped: bool = True):
        where = self.substation_table.c.name.like(name)
        return self._select_one_with_geometry(self.substation_table, where, mapped)  # type: ignore

    def get_substations_by_dist(self, dist: str, mapped: bool = True):
        return self._select_many_with_geometry(self.substation_table, self.substation_table.c.dist == dist, None, None, mapped)  # type: ignore

    def get_substation_by_poi(self, poi: tuple[float, float], mapped: bool = True):
        """
