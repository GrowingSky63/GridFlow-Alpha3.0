...
class RegionQueryMixin:
    ...
    Requer atributos definidos na classe principal:
      ...
      Métodos utilitários do GenericQueryMixin:
        _select_one_with_geometry
        _select_many_with_geometry
        _count_rows
    """
    engine: Engine
    region_table: Table
    ...
    def get_all_regions(
        ...
        return self._select_many_with_geometry(self.region_table, None, limit, offset, mapped)  # type: ignore

    # Filtros
    def get_region_by_id(self, id: int, mapped: bool = True):
        where = self.region_table.c.id == id
        return self._select_one_with_geometry(self.region_table, where, mapped)  # type: ignore

    def get_region_by_bdgd_id(self, bdgd_id: str, mapped: bool = True):
        where = self.region_table.c.bdgd_id == bdgd_id
        return self._select_one_with_geometry(self.region_table, where, mapped)  # type: ignore

    def get_region_by_cod_id(self, cod_id: str, mapped: bool = True):
        where = self.region_table.c.cod_id == cod_id
        return self._select_one_with_geometry(self.region_table, where, mapped)  # type: ignore

    def get_region_by_dist(self, dist: str | int, mapped: bool = True):
        where = self.region_table.c.dist == str(dist)
        return self._select_one_with_geometry(self.region_table, where, mapped)  # type: ignore

    def get_region_by_bdgd_name(self, bdgd_name: str, mapped: bool = True):
        where = self.region_table.c.bdgd_name.like(bdgd_name)
        return self._select_one_with_geometry(self.region_table, where, mapped)  # type: ignore

    def get_region_by_poi(self, poi: tuple[float, float], mapped: bool = True):
        # POINT(lon lat)
