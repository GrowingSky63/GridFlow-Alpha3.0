...
class BDGDDBInterface:
    ...
    def get_all_region(self, limit: int | None = None, offset: int | None = None) -> Sequence[Row[Any]]:
        with self.engine.begin() as conn:
            stmt = (
                select(self.region_table)
                .offset(offset)
    ...
    def get_region_by_id(self, id: int) -> Row[Any] | None:
        with self.engine.begin() as conn:
            stmt = (
                select(self.region_table)
                .where(self.region_table.c.bdgd_id == id)
                .limit(1)
            )
            return conn.execute(stmt).first()
        
    def get_region_by_bdgd_id(self, bdgd_id: str) -> Row[Any] | None:
        with self.engine.begin() as conn:
            stmt = (
                select(self.region_table)
                .where(self.region_table.c.bdgd_id == bdgd_id)
    ...
    def get_region_by_cod_id(self, cod_id: str) -> Row[Any] | None:
        with self.engine.begin() as conn:
            stmt = (
                select(self.region_table)
                .where(self.region_table.c.bdgd_id == cod_id)
                .limit(1)
            )
            return conn.execute(stmt).first()
        
    def get_region_by_dist(self, dist: str | int) -> Row[Any] | None:
        with self.engine.begin() as conn:
            stmt = (
                select(self.region_table)
                .where(self.region_table.c.bdgd_id == dist)
                .limit(1)
            )
            return conn.execute(stmt).first()
        
    def get_region_by_bdgd_name(self, bdgd_name: str) -> Row[Any] | None:
        with self.engine.begin() as conn:
            stmt = (
                select(self.region_table)
                .where(self.region_table.c.bdgd_id.like(bdgd_name))
                .limit(1)
            )
            return conn.execute(stmt).first()
