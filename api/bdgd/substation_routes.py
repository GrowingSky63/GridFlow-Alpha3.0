...
def get_region(
    ...
    if len(unique_params) == 0:
        # Listagem paginada
        return bdgd_manager.interface.get_all_substations(limit=limit, offset=offset)

    param_name, param_value = unique_params[0]

    match param_name:
        case "id":
            content = bdgd_manager.interface.get_substation_by_id(int(param_value))
        case "cod_id":
            content = bdgd_manager.interface.get_substation_by_cod_id(str(param_value))
        case "name":
            content = bdgd_manager.interface.get_substation_by_name(str(param_value))
        case "dist":
            content = bdgd_manager.interface.get_substations_by_dist(str(param_value))
        case "poi":
            try:
            ...
            content = bdgd_manager.interface.get_substation_by_poi(poi_coord)
        case _:
